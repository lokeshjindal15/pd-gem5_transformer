...
//#define DEF_FREQUENCY_UP_THRESHOLD		(30)//lokesh
...
int pdgem5_ondemand_flag[4] = { 0, 0, 0, 0}; 
...
static void od_check_cpu(int cpu, unsigned int load)
{
    ...
    if (pdgem5_ondemand_flag[cpu] == 1) // skip ondemand decision cycle this time
    {
	// NOPRINT printk (KERN_EMERG "##### ONDEMAND od_check_cpu skipping changing frequency for CPU *%d* this time since pdgem5_ondemand_flag[%d] is *%d* and making it zero\n", cpu, cpu*CACHELINE_SIZE, pdgem5_ondemand_flag[cpu*CACHELINE_SIZE]);
	pdgem5_ondemand_flag[cpu] = 0 ;
	return;
    }
	...
	if (load > od_tuners->up_threshold) {
		...
		if (policy->cur < policy->max){
			...
			//printk ("\n*****LOKESH Setting dbs_info->rate_mult=%d to od_tuners->sampling_down_factor= %d", dbs_info->rate_mult, od_tuners->sampling_down_factor);
		}
		//printk ("\n*****LOKESH dbs_freq_increse to policy->max = %d", policy->max);
